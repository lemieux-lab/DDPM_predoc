...
using HDF5
function tcga_fetch()
    fid = h5open("/home/golem/scratch/munozc/DDPM/full_TCGA_GE.h5", "r+")
    patients = read(fid, "case_id")

    cancer_types = read(fid, "cancer_type")
    genes=read(fid, "gene_id")

    dm_tmp100_ori = read(fid, "data_matrix")

    p_coding_genes = read(fid, "gene_type") .== "protein_coding"
    np_coding_genes = read(fid, "gene_type") .!= "protein_coding"

    dm_tmp100 = dm_tmp100_ori[:,p_coding_genes]

    close(fid)
    return dm_tmp100, patients, cancer_types
end
